...
If you changed alfred's port/IP mapping in docker-compose.yml (the "ports:"
...
def _alfred_bind_ip():
    """Best-effort scrape of alfred's host-side bind IP from the "ports:" mapping
    ...
    alfred_block = re.search(r"^\s*alfred:.*?(?=^\s{2}\S|\Z)", text, re.MULTILINE | re.DOTALL)
    if not alfred_block:
        return None

    port_line = re.search(r'-\s*"(\d+\.\d+\.\d+\.\d+):\d+:\d+"', alfred_block.group(0))
...
def warn_if_hosts_entries_are_stale():
    ...
    IP doesn't match alfred's current docker-compose.yml bind IP. This is exactly
    the failure mode of changing alfred's port/IP mapping without re-running
    start.py afterward: requests silently go nowhere instead of erroring loudly."""
    target_ip = _alfred_bind_ip()
    ...
    if stale:
        print()
        print(f"WARNING: alfred is configured to bind {target_ip}, but your hosts file still")
        print("points these \"-proxy\" entries somewhere else - requests to them will silently")
        print("fail to reach alfred:")
