...
- **"Edit in new tab" is a real second browser tab, not a modal**, backed by a small routing setup (`app.routes.ts`: `''` → `DashboardComponent`, `editor` → `JsonEditorPageComponent`) - `AppComponent` itself is now just a `<router-outlet>`. `nginx.conf` needs `try_files ... /index.html` for this: `/editor` is opened via `window.open` as a fresh HTTP request, not an in-app navigation, so without the SPA fallback nginx would 404 it. The two tabs talk over a `BroadcastChannel` named after a one-time session id (`shared/utils/editor-bridge.ts`); the starting text rides into the new tab via `sessionStorage`, which the browser clones into any same-origin tab opened via `window.open` - no backend round-trip needed just to seed the editor. `JsonPanelComponent.liveOverrideText` is the only thing an edit ever touches: it's read by `parsed()` ahead of the real `rawValue()` input when set, but the override is deliberately never written back into `CallsStateService` - cURL, comments, and the export dialog all keep reading the real call directly, so a hypothetical edit can't leak into a bug report.
