...
- **Every JSON block in a .md export is individually collapsible.** `codeBlock()`/`headersBlock()` in `markdown-builder.ts` wrap each fenced block in a GitHub-Flavored-Markdown `<details><summary>...</summary>` (collapsed by default, no `open` attribute) via the shared `collapsibleCodeBlock()` helper - the summary reads "Show JSON" for valid JSON and "Show details" otherwise, so a reader scanning a big multi-call report can expand only the payloads they actually need to look at. This is purely a presentation wrapper around the same never-truncated fenced content used everywhere else (single and bulk .md exports both go through this), so all the existing truncation-guard tests keep passing unchanged.
