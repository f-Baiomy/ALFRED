...
`CallsStateService` is the single facade for Live Calls: polls backend every 5s, merges in WebSocket pushes (live-first, deduped by `callKey`, pruned once a poll confirms them), and delegates all filtered/sorted/paginated/grouped/stats views to the shared `createCallListView` factory (`core/state/call-list-view.ts`). `SessionCycleDetailStateService` reuses the same factory over a cycle's captured calls (default sort `oldest-call`, vs. dashboard's `newest`).
