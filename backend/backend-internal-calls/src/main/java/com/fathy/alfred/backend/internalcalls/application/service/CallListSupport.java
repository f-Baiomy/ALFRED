...
/**
 * Search/filter/sort/paginate logic for any list of items that each wrap a CallRecord - mirrors
 * the frontend's shared/utils/call-utils.ts search/sort semantics exactly. Copied verbatim from
 * backend-calls' own CallListSupport (pure logic over the local CallRecord type, self-contained,
 * no cross-slice dependency issue) since this slice must stay fully isolated from backend-calls.
 */
