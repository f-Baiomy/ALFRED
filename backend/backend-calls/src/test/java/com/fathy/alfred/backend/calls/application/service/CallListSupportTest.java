...
class CallListSupportTest {
    ...
    void newestReversesTheSourceOrder() {
        List<CallRecord> source = List.of(call("a", "t", 1.0, 200, null), call("b", "t", 1.0, 200, null));
        var page = CallListSupport.apply(source, Function.identity(), "", "", "newest", 0, 10, true);
    ...
    void sortsByParsedTimestamp() {
        ...
        var oldest = CallListSupport.apply(source, Function.identity(), "", "", "oldest-call", 0, 10, true);
        assertThat(oldest.items()).extracting(CallRecord::url).containsExactly("unparseable", "early", "late");

        var newest = CallListSupport.apply(source, Function.identity(), "", "", "newest-call", 0, 10, true);
    ...
    void acceptsThePythonProxysOffsetTimestampFormat() {
        ...
        var page = CallListSupport.apply(source, Function.identity(), "", "", "oldest-call", 0, 10, true);
    ...
    void sortsByDuration() {
        ...
        var slowest = CallListSupport.apply(source, Function.identity(), "", "", "slowest", 0, 10, true);
        assertThat(slowest.items()).extracting(CallRecord::url).containsExactly("slow", "fast");

        var fastest = CallListSupport.apply(source, Function.identity(), "", "", "fastest", 0, 10, true);
    ...
    void sortsByStatusWithErrorsRankedWorst() {
        ...
        var page = CallListSupport.apply(source, Function.identity(), "", "", "status", 0, 10, true);
    ...
    void searchMatchesUrlCaseInsensitively() {
        ...
        var page = CallListSupport.apply(source, Function.identity(), "api.example", "", "newest", 0, 10, true);
    ...
    void filtersBySupplierHostname() {
        ...
        var page = CallListSupport.apply(source, Function.identity(), "", "a.com", "newest", 0, 10, true);
    ...
    void paginatesWithOffsetAndReportsTotalBeforePaging() {
        ...
        var page1 = CallListSupport.apply(source, Function.identity(), "", "", "oldest", 0, 2, true);
        assertThat(page1.items()).extracting(CallRecord::url).containsExactly("a", "b");
        assertThat(page1.total()).isEqualTo(4);

        var page2 = CallListSupport.apply(source, Function.identity(), "", "", "oldest", 2, 2, true);
    ...
    void offsetPastTheEndReturnsAnEmptyPageNotAnError() {
        ...
        var page = CallListSupport.apply(source, Function.identity(), "", "", "oldest", 50, 10, true);
        assertThat(page.items()).isEmpty();
        assertThat(page.total()).isEqualTo(1);
    }

    @Test
    void disabledPaginationIgnoresOffsetAndReturnsEverythingUpToLimit() {
        List<CallRecord> source = List.of(
                call("a", "t", 1.0, 200, null), call("b", "t", 1.0, 200, null),
                call("c", "t", 1.0, 200, null), call("d", "t", 1.0, 200, null)
        );
        var page = CallListSupport.apply(source, Function.identity(), "", "", "oldest", 2, 200, false);
        assertThat(page.items()).extracting(CallRecord::url).containsExactly("a", "b", "c", "d");
        assertThat(page.total()).isEqualTo(4);
    }

    @Test
    void disabledPaginationStillFiltersAndSorts() {
        List<CallRecord> source = List.of(call("https://a.com/x", "t", 1.0, 200, null), call("https://b.com/y", "t", 1.0, 200, null));
        var page = CallListSupport.apply(source, Function.identity(), "", "a.com", "newest", 5, 200, false);
        assertThat(page.items()).extracting(CallRecord::url).containsExactly("https://a.com/x");
    }

    @Test
    void disabledPaginationStillCapsAtLimit() {
        List<CallRecord> source = List.of(call("a", "t", 1.0, 200, null), call("b", "t", 1.0, 200, null), call("c", "t", 1.0, 200, null));
        var page = CallListSupport.apply(source, Function.identity(), "", "", "oldest", 0, 2, false);
        assertThat(page.items()).extracting(CallRecord::url).containsExactly("a", "b");
